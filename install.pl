...
if ($opt{symlink} && $opt{link}){
    die "Contradictory options:--link and --symlink (or -s).\nPlease pick one.";
};

if ($opt{symlink}) {
    if (eval { symlink("",""); 1 }) {
	$dup = sub { my ($a, $b) = @_; symlink($a, $b) };
    } else {
	warn "This system does not seem to support symlinks";
    }
};
...
if ($opt{link}) {
	...
	warn "This system does not seem to support links";
    }
};
...
#  die "$source is not a file" unless -f $source;
  ...
  if ($opt{compare} and compare($source, $target) == 0){
      ...
      if (-f $source
          && -f $target
          && $dev == (stat $target)[0] && $ino == (stat _)[1]
          && ! $opt{force})
      {
          warn "$source and $target are the same file";
      };

      my $copied = $dup->($source, $target);
      my $err = "$!";
      ...
      die "can't copy $source to $target: $err" unless $copied;
  ...
  if ($opt{preserve}){
    ...
    utime $atime, $mtime, $target or warn "can't utime $target: $!";
    my $oldmode = (07777 & $mode);
    chmod $oldmode, $target or warn "can't chmod $target: $!";
...
Creates I<target> as symlink to I<source> on systems that support symlinks.

=item B<--link>

Creates I<target> as another link to I<source> on systems that support
hard links.  Will fail if B<--symlink> is also included as an option.

=item B<--absolute>

Uses absolute path for each I<source>.  Normally, this has no real
effect, but it's useful for setting symlinks with B<-s>.
...
allow two or more install programs to work on the same files.
